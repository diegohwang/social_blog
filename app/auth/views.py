...
def before_request():
    #flask_login2.0用is_authenticated(), 3.0之后用is_authenticated
...
def unconfirmed():
    #flask_login2.0用is_anonymous(), 3.0之后用is_anonymous
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        #虽然程序末尾会自动提交，但生成token需要传入id值，所以先提交
        db.session.commit()
        token = user.generate_confirmation_token()
        send_email(user.email, 'Confirm Your Accout', 'auth/email/confirm',
                   user=user, token=token)
        flash('A confirmation eamil has been sent to you by email.')
        return redirect(url_for('main.index'))
...
#申请修改邮箱页面，发送修改页面链接(带token)到用户邮箱
