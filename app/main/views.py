...
def edit_profile():
    ...
    if form.validate_on_submit():
        current_user.name = form.name.data #此处current_user.name是表单forms的属性
        current_user.location = form.location.data
        current_user.about_me = form.about_me.data
        db.session.add(current_user)
        flash('Your profile has been updated.')
        return redirect(url_for('.user', username=current_user.username)) #此处current_user.username是models的属性
...
#设定show_followed cookie, '', 所有文章
@main.route('/all')
@login_required
def show_all():
    resp = make_response(redirect(url_for('.index')))
    resp.set_cookie('show_followed', '', max_age=0*24*60*60)
    return resp

#设定show_followed cookie, '1', 关注者文章
@main.route('/followed')
@login_required
def show_followed():
    resp = make_response(redirect(url_for('.index')))
    resp.set_cookie('show_followed', '1', max_age=0*24*60*60)
    return resp